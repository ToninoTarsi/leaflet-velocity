...
L.Control.Velocity = L.Control.extend({
    ...
    _onMouseMove: function _onMouseMove(e) {
        ...
        // Personalizzaizone Tony
        if (gridValue && !isNaN(gridValue[0]) && !isNaN(gridValue[1]) && gridValue[2]) {
            htmlOut = this.options.velocityType + " Direzione : <strong>" + self.vectorToDegrees(gridValue[0], gridValue[1], this.options.angleConvention).toFixed(0) + "°</strong>" + ", " + " Velocità: <strong>" + self.vectorToSpeed(gridValue[0], gridValue[1], this.options.speedUnit).toFixed(1) + " </strong> " + this.options.speedUnit;
        } else {
            htmlOut = this.options.emptyString;
        }
